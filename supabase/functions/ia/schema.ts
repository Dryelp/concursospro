...
const nullableNumberSchema = z.preprocess(numberOrNull, z.number().min(0).nullable()).default(null)
const nullableIntegerSchema = z
  .preprocess(integerOrNull, z.number().int().min(0).nullable())
  .default(null)
const confidenceSchema = z.preprocess(numberOrNull, z.number().min(0).max(1)).default(0.1)
...
export const extractionProviderValues = ['gemini', 'openrouter', 'heuristic', 'none'] as const
...
export const createEmptyEditalExtraction = (
  documentKind: z.infer<typeof editalExtractionSchema>['documentKind'] = 'desconhecido',
): EditalExtraction =>
